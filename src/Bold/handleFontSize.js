...
export function handleFontSize(event = null, context = null) {
    ...
    if (!lastClickedElement || !selectedTextTypes || selectedTextTypes.length === 0) {
        showNotification("Please click on a block first to detect text types", "error");
        return;
    }
    ...
    // Build CSS for each tag
    let css = "";

    selectedTextTypes.forEach(tag => {
        css += `
#${blockId} ${tag} strong {
    font-size: ${fontSize} !important;
}
`;
    ...
    showNotification("Font size applied to all detected bold texts!", "success");
}
