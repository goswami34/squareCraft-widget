...
export function handleBoldTextHighlightClick(
  event,
  lastClickedElement,
  ...
  const BoldtextHighlightPalate = event.target.closest(
    "#BoldtextHighlightPalate"
  );
  if (!BoldtextHighlightPalate) return;
  ...
  if (!colorPalette) {
    ...
    colorPalette.addEventListener("input", function (event) {
      ...
      targetElements.forEach((tag) => {
        ...
        if (boldElements.length > 0) {
          ...
          let styleTag = document.getElementById(
            `style-${block.id}-strong-highlight`
          );
          if (!styleTag) {
            styleTag = document.createElement("style");
            styleTag.id = `style-${block.id}-strong-highlight`;
          ...
          // Apply highlight color to strong tags using linear gradient
      ...
      context.showNotification(
        `✅ Text highlight applied to ${boldCount} Bold word(s) in ${selectedTextType}`,
        "success"
      );
    });
  }
