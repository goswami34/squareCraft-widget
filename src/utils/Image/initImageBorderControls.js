...
export function initImageBorderControls(selectedElement, context = {}) {
  ...
  function updateStylesToDOMAndDB(blockId, css) {
    addPendingModification(blockId, css, "image");
    saveModificationsforImage(blockId, css, "image");
  }
  ...
  function setupButton(button, type) {
    button.addEventListener("click", () => {
      ...
      const currentPosition = parseFloat(borderWidthBullet.style.left) || 0;
      const max = borderWidthSlider.offsetWidth;
      const width = Math.round((currentPosition / max) * 100);
      ...
      updateStylesToDOMAndDB(block.id, css);
      updateSliderPosition(width);
    });
  }

  [allButton, topButton, bottomButton, leftButton, rightButton].forEach(
    (btn, i) => {
      const type = ["all", "top", "bottom", "left", "right"][i];
      if (btn) setupButton(btn, type);
    }
  );
  ...
  ["Solid", "Dashed", "Dotted"].forEach((style) => {
    const id = `borderStyle${style}`;
    document.getElementById(id)?.addEventListener("click", () => {
      currentActiveBorderStyle = style.toLowerCase();
      const selected = document.querySelector(".sc-selected-image");
      if (!selected) return;
      const block = selected.closest('[id^="block-"]');
      if (!block) return;
  ...
  if (radiusSlider && radiusBullet && radiusFill && radiusDisplay) {
    ...
    const drag = (e) => {
      ...
      offsetX = Math.max(bulletRadius, Math.min(offsetX, max - bulletRadius));
      const percent = offsetX / max;
      const radius = Math.round(percent * 100);
    ...
    radiusBullet.addEventListener("mousedown", () => (dragging = true));
    radiusBullet.addEventListener("touchstart", () => (dragging = true));
