...
// Export the applyBorderColorFromPalette function
export function applyBorderColorFromPalette(
  color,
  alpha = 1,
  selectedElement,
  ...
  const block = currentElement.closest('[id^="block-"]');
  if (!block) return;
  ...
  if (match) {
    ...
    declarations = declarations.replace(/border-color\\s*:\\s*[^;]+;?/g, "");
  ...
  // Update local state
  selectedBorderColor = rgbaColor;
  const colorCode = document.getElementById("color-code");
  if (colorCode) {
    colorCode.textContent = rgbaColor;
  }

  // Save to database if function provided
  if (typeof saveImageBorderModifications === "function") {
    const stylePayload = {
      image: {
        selector: blockSelector,
        styles: { "border-color": rgbaColor },
      },
    };
    saveImageBorderModifications(blockId, stylePayload, "image");
  }
  ...
  console.log("🎨 Applied border color:", rgbaColor, "to block:", blockId);
}
