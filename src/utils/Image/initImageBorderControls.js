...
// ✅ Full fixed initImageBorderControls function with correct database saving logic
...
export function initImageBorderControls(selectedElement, context = {}) {
  ...
  function setActiveBorderButton(activeBtn) {
    ...
    if (activeBtn !== allButton)
      allButton.classList.add("sc-blur-sm", "sc-pointer-events-none");
    else allButton.classList.remove("sc-blur-sm", "sc-pointer-events-none");
  }
  ...
  function updateStyleElement(blockId, borderWidth) {
    ...
    if (window.__scActiveBorderType === "all") {
      allBorderWidth = borderWidth;
      newCSS = `${blockSelector} {\n  border-width: ${borderWidth}px;\n  box-sizing: border-box;\n  border-style: solid;\n  ${
        selectedBorderColor ? `border-color: ${selectedBorderColor};` : ""
      }\n}`;
    } else {
      const property = sideMap[window.__scActiveBorderType];
      if (!property) return;
      const widthValue = `${borderWidth}px !important;`;

      if (window.__scActiveBorderType === "top") topBorderWidth = borderWidth;
      if (window.__scActiveBorderType === "bottom")
        bottomBorderWidth = borderWidth;
      if (window.__scActiveBorderType === "left") leftBorderWidth = borderWidth;
      if (window.__scActiveBorderType === "right")
  ...
  if (colorCode) {
    const observer = new MutationObserver(() => {
      const newColor = colorCode.textContent.trim();
      if (!newColor || newColor === "Select") return;
      selectedBorderColor = newColor;
  ...
  // Button setup and drag logic here (unchanged)
  // radius control logic remains as you already structured correctly

  // Apply border style click handlers
  document
    .getElementById("borderStyleSolid")
    ?.addEventListener("click", () => applyBorderStyle("solid"));
  document
