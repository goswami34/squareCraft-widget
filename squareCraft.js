...
(async function squareCraft() {
  ...
  // A more organized click handler to prevent spamming.
  function mainClickListener(event) {
    const target = event.target;

    // --- Palette Triggers ---
    if (target.closest("#border-color-select")) {
      setTimeout(() => initBorderColorPaletteToggle(themeColors), 100);
      return;
    }
    if (target.closest("#buttonFontColorPalate")) {
      const palette = document.getElementById("button-font-color-palette");
      if (palette) {
        palette.classList.toggle("sc-hidden");
        setTimeout(() => initButtonFontColorPaletteToggle(themeColors, selectedElement), 50);
      }
      return;
    }
    if (target.closest("#overLayFontColorPalate")) {
      const palette = document.getElementById("overlay-color-palette");
      if (palette) {
        palette.classList.toggle("sc-hidden");
        setTimeout(() => initOverLayColorPalate(themeColors, () => selectedElement), 50);
      }
      return;
    }

    // --- Block Interaction Logic ---
    const clickedBlock = target.closest('[id^="block-"]');
    if (clickedBlock) {
      // Set the currently selected element for the widget to use.
      ...
      // Detect what's inside the block (image, text, button) to show the right tabs.
      waitForElement("#typoSection, #imageSection, #buttonSection")
        .then(() => detectBlockElementTypes(clickedBlock))
        .catch((error) => console.error(error.message));
      
      // Initialize controls for different sections within the widget
      setTimeout(initImageSectionControls, 100);
      setTimeout(initButtonSectionToggleControls, 100);
    }
  ...
  // Remove old listener and add the new, cleaner one.
  document.body.removeEventListener("click", mainClickListener); //
