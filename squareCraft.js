...
// Add this code to track previous font sizes
const previousFontSizes = new Map();

// Modify the font size input handler to store previous sizes
document.getElementById("squareCraftFontSize").addEventListener("input", async function() {
  if (!SelectionManager.selectedParagraph || !SelectionManager.selectedLink) {
    console.warn("⚠️ Please select a link first");
    return;
  ...
  try {
    ...
    for (const anchor of allAnchors) {
      ...
      // Store the previous font size before changing
      const currentSize = window.getComputedStyle(anchor).fontSize;
      previousFontSizes.set(anchor.id, currentSize);
...
// Add the reset functionality for font size
document.querySelector(".underline-element-font-size").addEventListener("click", async function() {
  ...
  try {
    ...
    // Apply the previous font size to all anchors
    for (const anchor of allAnchors) {
      // Ensure anchor has an ID
      if (!anchor.id) {
        anchor.id = `link-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      ...
      const previousSize = previousFontSizes.get(anchor.id);
      if (!previousSize) {
        console.warn("⚠️ No previous font size found for this element");
        continue;
      }
      ...
      // Update styles with previous font size
      const updatedStyles = {
        ...existingStyles,
        'font-size': previousSize
      };
      ...
      // Update the font size input to match the previous size
      const numericSize = parseInt(previousSize);
      if (!isNaN(numericSize)) {
        document.getElementById("squareCraftFontSize").value = numericSize;
      }
    }

    console.log("✅ Font size reset to previous value for all links in paragraph");
  } catch (error) {
