            ...
            if (storedPageId === pageId) {
                elements.forEach(({ elementId, css, elementStructure }) => {
                    if (css && css.span && elementStructure) {
                        let parentElement = document.getElementById(elementStructure.parentId);
                        if (!parentElement) return;

                        // ✅ Restore full paragraph content first
                        parentElement.innerHTML = elementStructure.fullContent;

                        // ✅ Find the exact word and wrap it in a span
                        let updatedContent = parentElement.innerHTML.replace(
                            new RegExp(`\\b${elementStructure.content}\\b`, "g"),
                            `<span id="${css.span.id}" class="squareCraft-font-modified" style="font-size: ${css.span["font-size"]};">${elementStructure.content}</span>`
                        );

                        parentElement.innerHTML = updatedContent;
                        console.log(`✅ Recreated modified word inside paragraph: ${elementStructure.content}`);
                    }
                });
            }
        });
...
async function saveModifications(elementId, css, elementStructure = null) {
    ...
    // Get the full content of the parent paragraph
    const container = document.getElementById(elementId)?.parentElement;
    const fullContent = container ? container.innerHTML : null;
        ...
        modifications: [{
            ...
            elements: [{
                ...
                elementStructure: elementStructure || {
                    ...
                    fullContent: fullContent // ✅ Store the full paragraph content
                }
            }]
        }]
    };
