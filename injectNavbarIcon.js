...
export function injectNavbarIcon() {
  ...
  function insertToolbarIcon() {
    // First, try to find the specific tidILMJ7AVANuKwS toolbar container
    const specificToolbar = parent.document.querySelector(".tidILMJ7AVANuKwS");

    if (specificToolbar && !specificToolbar.querySelector(".sc-toolbar")) {
      // Create toolbar for the specific tidILMJ7AVANuKwS container
      createToolbarInContainer(specificToolbar);
    }

    // Then look for other toolbar containers, but exclude the one we already processed
    const otherToolbarContainers = parent.document.querySelectorAll(
      "div[data-block-toolbar='true']:not(.tidILMJ7AVANuKwS), div[role='menu']:not(.tidILMJ7AVANuKwS)"
    );

    otherToolbarContainers.forEach((toolbarContainer) => {
      if (!toolbarContainer.querySelector(".sc-toolbar")) {
        createToolbarInContainer(toolbarContainer);
      }
    });
  }

  function createToolbarInContainer(toolbarContainer) {
    const iconSrc =
      localStorage.getItem("sc_icon") ||
      "https://fatin-webefo.github.io/squareCraft-plugin/public/squarecraft-only-logo.svg";
